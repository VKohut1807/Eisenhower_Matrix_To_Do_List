  ...
  <div class="end">
    ...
    <div class="withoutTask">
      ...
      class="completedOrNot">
        <span>
          <input id="cb1" type="checkbox" disabled checked />
          <label for="cb1"></label>
        </span>
      </span>
      <span
      v-else-if="filter === 'notCompleted'
      || filter === 'noneN'
      || filter === 'redN'
      || filter === 'greenN'
      || filter === 'yellowN'
      ...
      <span v-else class="completedOrNot">
        <span>
          <input id="cb1" type="checkbox" disabled checked />
          <label for="cb1"></label>
        </span>
      ...
      || filter === 'noneC'
      || filter === 'noneN'"
      class="textTask">{{'None'}}</span>
      <span
      v-else-if="filter === 'red'
      || filter === 'redC'
      || filter === 'redN'"
      class="textTask">{{'Red'}}</span>
      <span
      v-else-if="filter === 'green'
      || filter === 'greenC'
      || filter === 'greenN'"
      class="textTask">{{'Green'}}</span>
      <span
      v-else-if="filter === 'yellow'
      || filter === 'yellowC'
      || filter === 'yellowN'"
      class="textTask">{{'Yellow'}}</span>
      <span
      v-else-if="filter === 'blue'
      || filter === 'blueC'
      || filter === 'blueN'"
      class="textTask">{{'Blue'}}</span>
      <span
      v-else class="textTask">{{'All'}}</span>
      <span class="textTask">Tasks</span>
    </div>
    <div>!</div>
  </div>
  <div class="author">
    <span class="arrow">
      <a class="by">animation by - "Hayakawa"</a>&#9655;
    </span>
  </div>
...
<script>
export default {
  props: ["filter", "sampleAssignments"],
}
</script>
...
<style scoped>
  ...
  .completedOrNot input[type="checkbox"] {display: none;}

  .completedOrNot label:before {
      height: 1em;
      border-radius: .1em;
      width: 1em;
      display: block;
  }
  .completedOrNot input[type="checkbox"] + label:before {content: '';}

  .completedOrNot input[type="checkbox"]:disabled + label:before {
      content: '🗴';
      color: red;
      background-color: white;
  }

  .completedOrNot input[type="checkbox"]:checked + label:before {
    content: '✓';
    color: green;
    background-color: white;
  }
  ...
  @keyframes moveSqu_01 {
    ...
    100% { transform: translateY(-10em); border-radius: 0;}
  }
